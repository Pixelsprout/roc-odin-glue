...
Roc_Init_Points :: struct {
	x: f32,
	y: f32,
}

#assert(size_of(Roc_Init_Points) == 8)
#assert(align_of(Roc_Init_Points) == 4)
#assert(offset_of(Roc_Init_Points, x) == 0)
#assert(size_of(type_of(Roc_Init_Points{}.x)) == 4)
#assert(offset_of(Roc_Init_Points, y) == 4)
#assert(size_of(type_of(Roc_Init_Points{}.y)) == 4)

Roc_Step_Arg0 :: Roc_Init
Roc_Step_Arg0_Points :: Roc_Init_Points
Roc_Step :: Roc_Init
Roc_Step_Points :: Roc_Init_Points

roc_init_decref :: proc(value: Roc_Init) {
	roc_list_decref_roc_init_points(value.points)
}

roc_list_decref_roc_init_points :: proc(list: Roc_List(Roc_Init_Points)) {
	roc_list_decref_flat(list)
}

roc_list_from_slice_roc_init_points :: proc(elems: []Roc_Init_Points) -> Roc_List(Roc_Init_Points) {
...
roc_list_decref :: proc {
	roc_list_decref_roc_init_points,
}

roc_list_from_slice :: proc {
	roc_list_from_slice_roc_init_points,
}
